...
const tripData = [
  { day: 1, dateStr: "25", dayOfWeek: "WED", title: "啟程．飛往維也納", locationName: "Taipei", imageUrl: "https://images.unsplash.com/photo-1436491865332-7a61a109cc05?q=80&w=2074&auto=format&fit=crop", hourlyWeather: generate24HourWeather(22, 'cloudy'), items: [{ time: "20:35", title: "桃園機場集合", category: "transport", description: "第二航站長榮航空 20 號後方櫃檯。", outfit: "機上穿搭建議層次穿法。" }, { time: "23:35", title: "台北/維也納", category: "flight", description: "BR065 長榮航空。" }] },
  { day: 2, dateStr: "26", dayOfWeek: "THU", title: "哈爾斯塔特", locationName: "Salzburg", imageUrl: "https://image.kkday.com/v2/image/get/s1.kkday.com/product_36568/20200122064534_xxRQH/jpg", hourlyWeather: generate24HourWeather(4, 'cloudy'), items: [{ time: "10:00", title: "Hallstatt 小鎮", category: "sightseeing", description: "世界最美湖畔。", aiTips: [{type:'must-buy', content:'天然岩鹽'}, {type:'must-eat', content:'湖區鱒魚'}] }] },
  { day: 3, dateStr: "27", dayOfWeek: "FRI", title: "國王湖與庫倫洛夫", locationName: "Český Krumlov", imageUrl: "https://blog-static.kkday.com/zh-tw/blog/wp-content/uploads/shutterstock_711751918.jpg", hourlyWeather: generate24HourWeather(2, 'rainy'), items: [{ time: "09:00", title: "國王湖 Königssee", category: "sightseeing", description: "最清澈的高山湖泊。" }] },
  ...
  { day: 5, dateStr: "01", dayOfWeek: "SUN", title: "布拉格天文鐘", locationName: "Prague", imageUrl: "https://photo.settour.com.tw/900x600/https://www.settour.com.tw/ss_img/info/location/PRG/G0/PRG0000011/PRG0000011_45988.jpg", hourlyWeather: generate24HourWeather(7, 'sunny'), items: [{ time: "14:30", title: "老城區漫步", category: "sightseeing", aiTips: [{type:'luxury', content:'巴黎大街精品'}, {type:'must-buy', content:'菠丹妮手工皂'}] }] },
  ...
  { day: 9, dateStr: "05", dayOfWeek: "THU", title: "再見維也納", locationName: "Vienna Airport", imageUrl: "https://images.unsplash.com/photo-1569154941061-e231b4725ef1?q=80&w=2070&auto=format&fit=crop", hourlyWeather: generate24HourWeather(12, 'cloudy'), items: [{ time: "12:05", title: "維也納機場", category: "transport" }] },
  { day: 10, dateStr: "06", dayOfWeek: "FRI", title: "抵達台北", locationName: "Taipei", imageUrl: "https://images.unsplash.com/photo-1470004914212-05527e49370b?q=80&w=2074&auto=format&fit=crop", hourlyWeather: generate24HourWeather(25, 'sunny'), items: [{ time: "06:40", title: "抵達家園", category: "transport" }] }
];
...
const Tag = ({ type, content }: { type: string; content: string }) => {
  ...
  return (
    <span className="inline-flex items-center text-[10px] px-2 py-0.5 rounded-full border mr-2 mb-2" 
...
const ExpenseTracker = ({ user }: { user: any }) => {
  ...
  return (
    <div className="p-6 pb-24 animate-fade-in text-[#4A4641]">
      <div className="mb-8">
        ...
        <div className="flex items-center gap-2 mt-2">
            ...
            <span className="text-[10px] text-gray-400">均攤: NT$ {Math.round(expenses.reduce((s, e) => s + e.twdAmount, 0) / 4).toLocaleString()}</span>
        </div>
      </div>
      <div className="bg-white p-5 rounded-2xl border border-[#E8E6E1] space-y-4 mb-8">
        <input type="text" placeholder="項目名稱" value={item} onChange={e => setItem(e.target.value)} className="w-full bg-[#F5F3F0] rounded-xl px-4 py-3 text-sm focus:outline-none" />
        <div className="flex gap-2">
          ...
          <select value={currency} onChange={e => setCurrency(e.target.value)} className="bg-[#F5F3F0] rounded-xl px-2 text-sm outline-none">
            <option value="EUR">€</option><option value="CZK">Kč</option>
          </select>
        </div>
        <div className="flex justify-between px-2">
          {['R', 'C', 'N', 'O'].map(p => (
            <button key={p} onClick={() => setPayer(p)} className={`w-9 h-9 rounded-full text-xs transition-all ${payer === p ? payersInfo[p].color : 'bg-[#F5F3F0] text-gray-400'}`}>{p}</button>
          ))}
        </div>
        <button onClick={add} className="w-full bg-[#4A4641] text-white py-3 rounded-xl text-sm tracking-widest flex items-center justify-center gap-2">
            <Plus size={16} /> 記一筆
        </button>
      </div>
        ...
        {expenses.map(e => (
          <div key={e.id} className="flex justify-between items-center bg-white p-4 rounded-xl border border-[#E8E6E1]">
            <div className="flex items-center gap-3">
              ...
              <div><div className="text-sm">{e.item}</div><div className="text-[10px] text-gray-400">{e.currency} {e.amount}</div></div>
            </div>
            <div className="flex items-center gap-3">
...
export default function App() {
  ...
  const renderContent = () => {
    ...
    if (view === 'info') return (
        <div className="p-8 animate-fade-in space-y-10">
            <div className="bg-[#EFECE6] h-40 rounded-3xl flex items-center justify-center border border-[#E5E3DB]">
                <Globe size={32} className="text-[#A65D57] opacity-20" />
            </div>
            <section className="space-y-5">
                ...
                <div className="flex justify-between items-center bg-rose-50/30 p-5 rounded-2xl border border-rose-100">
                    <span className="text-sm font-light text-rose-800">外交部緊急助救</span>
                    <ShieldAlert size={16} className="text-rose-400"/>
                </div>
            </section>
            <section className="space-y-4">
                <h3 className="text-xs tracking-[0.2em] text-gray-400 border-b pb-2 flex items-center gap-2"><CalendarDays size={14}/> 旅行日誌</h3>
                <div className="text-[10px] text-gray-400 leading-relaxed italic">
                    Austria & Czech 2026 Family Trip. <br/>紀錄每一刻的美好時光。
                </div>
            </section>
      ...
      <div className="animate-fade-in pb-20">
        <div className="relative mx-5 mt-5 h-64 rounded-[2rem] overflow-hidden shadow-sm">
          ...
          <div className="absolute bottom-6 left-6 text-white">
            <div className="text-[10px] tracking-[0.3em] opacity-70 mb-1">DAY {cur.day} · {cur.locationName}</div>
              ...
              <div key={i} className="flex group">
                ...
                <div className="flex-1 pb-2">
                  <h4 className="text-base font-light text-[#4A4641] flex items-center gap-2 mb-1">
                    {item.title}
                    <a href={`https://www.google.com/maps/search/?api=1&query=${item.title}`} target="_blank" rel="noreferrer" className="text-gray-300 hover:text-[#A85A46]"><MapPin size={12}/></a>
                  </h4>
  ...
  return (
    <div className="max-w-md mx-auto bg-[#F5F3F0] min-h-screen relative shadow-2xl overflow-hidden font-sans font-light">
      <style>{`.no-scrollbar::-webkit-scrollbar { display: none; } .animate-fade-in { animation: f 0.6s ease-out; } @keyframes f { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }`}</style>
